package com.chandra.myflickr.misc;

public class Constants {

    public static final String PREFS_NAME = "jayplabs-myflickr-pref"; //$NON-NLS-1$
    public static final String CALLBACK_SCHEME = "jayplabs-myflickr-oauth"; //$NON-NLS-1$
    public static final String KEY_OAUTH_TOKEN = "jayplabs-myflickr-oauthToken"; //$NON-NLS-1$
    public static final String KEY_TOKEN_SECRET = "jayplabs-myflickr-tokenSecret"; //$NON-NLS-1$
    public static final String KEY_USER_NAME = "jayplabs-myflickr-userName"; //$NON-NLS-1$
    public static final String KEY_USER_ID = "jayplabs-myflickr-userId"; //$NON-NLS-1$

    public static final int NUM_OF_PHOTOS_PER_PAGE = 50;
    public static final int NUM_OF_PAGES = 1;
}
